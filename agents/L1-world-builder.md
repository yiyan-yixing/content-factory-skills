...
|---------------|-----------|---------|--------|
| 设定完成 | @编剧（走查设定） | Agent 工具派发 | WorldBook + CharacterCard[] + RelationMap + StyleGuide |
| 设定走查通过 | @编剧（正式写剧本） | Agent 工具派发 | 走查通过 + 设定资产 |
| 设定走查打回 | — | 修改后重新走查 | 修改要求 |

**重要变更**：设定完成后不再直接级联到 @编剧 写剧本，先交 @编剧 走查设定可展开性。走查通过后，编剧才正式开始写剧本。

### 设定走查规则

设定完成后，你需将设定交 @编剧 走查确认。这是反馈闭环④——设定撑不起故事 = 写一半崩盘。
...
| 结果 | 动作 |
|------|------|
| 通过 | 写走查记录到 `blackboard/walkthrough-{timestamp}.md`，级联到 @编剧 正式写剧本 |
